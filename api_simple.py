...
@bot.tree.command(name="notifyall", description="Send notification to all active players (Owner Only)")
@app_commands.describe(message="The message to send to all players")
async def notifyall(interaction: discord.Interaction, message: str):
    if not await check_security(interaction): return
    cursor = bot.db.cursor()
    
    # Insert notification for all players
    cursor.execute("INSERT INTO notifications (target_type, target_key, message) VALUES (?, ?, ?)", 
                   ("all", None, message))
    bot.db.commit()
    ...
    await interaction.response.send_message(
        f"✅ Notification sent to all players!\n**Message:** {message}\n**Active Players:** {active_count}",
        ephemeral=True
    )

@bot.tree.command(name="notifykey", description="Send notification to a specific key (Owner Only)")
@app_commands.describe(key="The key to send notification to", message="The message to send")
async def notifykey(interaction: discord.Interaction, key: str, message: str):
    ...
    # Insert notification for specific key
    cursor.execute("INSERT INTO notifications (target_type, target_key, message) VALUES (?, ?, ?)", 
                   ("key", key, message))
    bot.db.commit()
...
@bot.tree.command(name="joinplayer", description="Join a player's game by their key (Owner Only)")
@app_commands.describe(target_key="The key of the player to join", requester_key="Your key (optional)")
async def joinplayer(interaction: discord.Interaction, target_key: str, requester_key: str = None):
    if not await check_security(interaction): return
    cursor = bot.db.cursor()
    
    # Check if target key exists
    cursor.execute("SELECT key, redeemed_by, is_redeemed FROM keys WHERE key = ?", (target_key,))
    row = cursor.fetchone()
    
    if not row:
        return await interaction.response.send_message("❌ Target key not found.", ephemeral=True)
    
    if row[2] == 0:
        return await interaction.response.send_message("❌ Target key hasn't been redeemed yet.", ephemeral=True)
    
    # If no requester key provided, get the first key for this user
    if not requester_key:
        cursor.execute("SELECT key FROM keys WHERE redeemed_by = ? AND is_redeemed = 1 LIMIT 1", (interaction.user.id,))
        req_row = cursor.fetchone()
        if not req_row:
            return await interaction.response.send_message("❌ You don't have any redeemed keys. Please provide a requester_key.", ephemeral=True)
        requester_key = req_row[0]
    
    # Generate unique job ID
    job_id = str(uuid.uuid4())
    
    # Insert join request
    cursor.execute("INSERT INTO join_requests (requester_key, target_key, job_id) VALUES (?, ?, ?)", 
                   (requester_key, target_key, job_id))
    bot.db.commit()
    
    target_user_id = row[1]
    target_mention = f"<@{target_user_id}>" if target_user_id else "Unknown user"
    
    await interaction.response.send_message(
        f"✅ Join request created!\n**Your Key:** `{requester_key}`\n**Target Key:** `{target_key}`\n**Target User:** {target_mention}\n\n*You will join their game shortly...*",
        ephemeral=True
    )
